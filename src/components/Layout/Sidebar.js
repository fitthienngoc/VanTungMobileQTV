...
const SidebarItem = ({ item, isActive,submenu }) => (
    <li style={submenu} className={isActive ? 'active' : ''}>
...
const SidebarSubItem = ({ item, isActive, handler, children, isOpen,submenu }) => (
    <>
        <li style={submenu} className={isActive ? 'active' : ''}>
                                                ...
                                                <SidebarSubItem  item={item} isOpen={this.state.collapse[item.name]} handler={this.toggleItemCollapse.bind(this, item.name)} isActive={this.routeActive(this.getSubRoutes(item))} key={i}>
                                                    <SidebarSubHeader item={item} key={i} />
                                                    {
                                                        item.submenu.map((subitem, i) =>
                                                            <SidebarItem submenu={{paddingLeft:15}} key={i} item={subitem} isActive={this.routeActive(subitem.path)} />
                                                        )
                                                    }
                                                </SidebarSubItem>
                                            ]
